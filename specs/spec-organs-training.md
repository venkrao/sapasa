...
This module provides a dedicated section of the app covering the **physical organs involved in singing**, their role, and practical exercises to train them. It is separate from pitch exercises — it addresses the *instrument* (the body) rather than the *music* (the notes). Content is static and informational — no microphone or pitch detection is involved.
...
type TrainingExercise = {
  name: string
  description: string
  duration?: string         // e.g. "30 seconds", "5 minutes daily"
...
type VocalOrgan = {
  id: string
  name: string
  emoji: string             // visual anchor for UI
  role: string              // one sentence — what it does in singing
  whyItMatters: string      // one sentence — consequence of weakness
  exercises: TrainingExercise[]
  carnaticRelevance: string // how this organ specifically affects Carnatic practice
}
```

***

## 3. Full Organ Definitions
...
export const VOCAL_ORGANS: VocalOrgan[] = [
  ...
  {
    id: 'core',
    name: 'The Abdominal & Core Muscles',
    emoji: '💪',
...
A dedicated **"Voice Training"** screen — separate from the pitch monitor and exercise panel — with:

- One card per organ, collapsed by default
- Card header: emoji + name + one-line role summary
- Expanded card: why it matters → Carnatic relevance → exercises list
- Each exercise shows name, description, duration, difficulty badge
- No microphone usage — purely informational

This screen does not compete with the pitch monitor for screen real estate and can be accessed from a top-level navigation tab.

***

## 5. Relationship to Pitch Exercises

Several exercises in this module directly complement specific pitch exercises:

| Vocal organ exercise | Pairs with |
|---|---|
| Sustained SSS exhale | Sa Pa Sa — tests sustain endurance |
| Lip trills on scales | Sarali Varisai — warms up folds before drilling pitch |
| Ma-Me-Mi-Mo-Mu patterns | Sarali Varisai — vowel articulation on known pitches |
| Straw phonation | Any exercise — post-session cool-down |
| Tongue forward placement | Daatu Varisai — precision leaps require relaxed tongue |

A future version could surface these pairings contextually — e.g. before starting Sarali Varisai, the app could suggest "Warm up with lip trills first". [themystickeys](https://themystickeys.com/how-to-develop-strong-breath-control-for-carnatic-singing/)
